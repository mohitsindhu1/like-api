...
ACCOUNTS = {
    ...
    'BD': "uid=4125700859&password=5945A4FC6E3ED5E0A22E4FE69B84218009D8F96E2CF0DB6343E1E75D7F2B2D50",
...
async def detect_uid_region(uid: str) -> str:
    if str(uid) in UID_REGION_CACHE:
        return UID_REGION_CACHE[str(uid)]
    
    for region in PRIORITY_REGIONS:
        try:
            result = await GetAccountInformation(uid, "0", region, "/GetPlayerPersonalShow", auto_detect=False)
            if isinstance(result, dict) and not result.get("error"):
                if "basicInfo" in result:
                    basic_info = result.get("basicInfo", {})
                    if isinstance(basic_info, dict):
                        nickname = basic_info.get("nickname", "")
                        uid_str = str(basic_info.get("accountId", ""))
                        if nickname and nickname != f"Player_{uid}" and uid_str == str(uid):
                            UID_REGION_CACHE[str(uid)] = region
                            return region
...
async def GetAccountInformation(ID, UNKNOWN_ID, regionMain, endpoint, auto_detect=True):
    json_data = json.dumps({
        "a": ID,
        "b": UNKNOWN_ID
    })
    ...
    if auto_detect and (not regionMain or regionMain.upper() not in SUPPORTED_REGIONS):
        detected_region = await detect_uid_region(ID)
        if detected_region is None:
            return {
                "error": "Region detection failed",
                "message": f"Could not detect region for UID {ID}. Please specify region manually."
            }
        regionMain = detected_region
    ...
    token, region, jwt_serverUrl = await create_jwt(regionMain)
    
    if jwt_serverUrl and jwt_serverUrl != "0":
        serverUrl = jwt_serverUrl
    else:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if auto_detect and isinstance(message, dict) and "basicInfo" in message:
                message["detected_region"] = regionMain
            return message
    except Exception as e:
        return {
            "error": "Request failed",
            "message": f"Failed to fetch account information: {str(e)}"
        }
...
async def main():
    while True:
        ...
        if choice == '1':
            ...
            print(f"\nSupported regions: {', '.join(SUPPORTED_REGIONS)}")
            region = input("Enter the region (press Enter to auto-detect from UID): ").upper()
            ...
            try:
                ...
                if not region or region.strip() == "":
                    print(f"🔍 Auto-detecting region for UID {player_id}...")
                    info = await GetAccountInformation(player_id, unknown_id, "", endpoint)
                    if info.get("error") == "Region detection failed":
                ...
                if info.get("error"):
                    ...
                    if info.get("detected_region"):
                        print(f"\n✅ Region auto-detected: {info['detected_region']}")
                    print("\n--- Account Information ---")
